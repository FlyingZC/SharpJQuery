...
//===================定义全局参数========================
var citylinkageParams = {//封装级联选择城市的信息
	//被选中的城市等级ID
	cityItemsKey: {
		provincesLevelId: 0, //默认值,省
		cityLevelId: null,//市
		countyLevelId: null//县
	},
	//被选中城市信息
	selectedCity: {
		provinces: {
...
citylinkage.prototype = {
	...
	registerContentClick: function() {
		...
		var handleEvent = commonOp.delegate(this._handlerContentClick, this, [{ contentId: contentId }]);
	...
	registerDocumentClick: function() {
		...
		$(document).click(function(event) {
			var handleEvent = commonOp.delegate(citylinkage.prototype._handleDocumentClick, this, [{ event: event, options: options }]);
			handleEvent();
		});
	},
	...
	registerProvincestabClick: function() {
		var contentId = this.options.contentId;
		var handleEvent = commonOp.delegate(this._handlerProvincestabClick, this, [{ contentId: contentId }]);
		$("#provincestab_" + contentId).click(handleEvent);
	},

	registerCitytabClick: function() {
		var contentId = this.options.contentId;
		var handleEvent = commonOp.delegate(this._handlerCitytabClick, this, [{ contentId: contentId }]);
		$("#citytab_" + contentId).click(handleEvent);
	},

	registerCountytabClick: function() {
		var contentId = this.options.contentId;
		var handleEvent = commonOp.delegate(this._handlerCountytabClick, this, [{ contentId: contentId }]);
	...
	registertabbodydetailInfoClick: function() {
		...
		$(".tab-body-detailInfo").click(function(event) {
			...
			var handleEvent = commonOp.delegate(obj._handlertabbodydetailInfoClick, obj, [{ contentId: contentId, attrId: attrId, attrItem: attrItem }]);
			handleEvent();
		});
	},
	...
	_handlerContentClick: function() {
		var contentId = this.options.contentId;
		if($("#citylinkageTab").length > 0) {//下拉显示省市县的tab页面
			return;
		}
	...
	//获取元素位置  定位,根据容器div的坐标 定位出下拉的省市县tab的坐标
